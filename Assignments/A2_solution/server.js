...
Also songs to save are sent to the server as an array on lines (strings).
Both exchanges are done as JSON strings.

The URL path portion of the POST message is used to determine whether it is a
request to fetch a song or a request to save a song.

The server is not aware that the client is extracting chords from
the lyric lines.

To make this even simpler on the server side, it could have just sent the
entire file as a string and let the client carve it up as it wants.
*/
...
http.createServer(function(request, response) {
  ...
  request.on("end", function() {
    ...
    if (request.method == "POST" && urlObj.pathname === "/fetchSong") {
      ...
      let songFilePath = "" //path to requested song file
      ...
      //end fetch song
      //a POST request to save song to the file system
      let songLines = dataObj.songLines
      let saveAsFileName = dataObj.saveAsFileName
      let saveAsFilePath = "songs/" + saveAsFileName + ".txt"
      let fileDataString = ""
      ...
      fs.writeFile(saveAsFilePath, fileDataString, function(err, data) {
        ...
        response.end(JSON.stringify(returnObj))
        //add new song file to servers list of songs
