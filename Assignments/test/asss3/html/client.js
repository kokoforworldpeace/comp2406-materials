...
for (let ball of allBalls) {
  if (dataObj&&dataObj.name===ball.name) {
...
function handleTimer() {//trigger every 100
  let ball_coor = {}
    for(let i=0;i<allBalls.length;i++){
      let ballH = allBalls[i]
      if(ballH.x&&ballH.y){

    if (ballH.x_speed!==0||ballH.y_speed!==0) {
      
    ball_coor.name=ballH.name,
    ball_coor.x=ballH.x,
    ball_coor.y=ballH.y,
      //send when exits ball move
      console.log(`emit:${JSON.stringify(ball_coor)}`)
...
function handleSubmit() {
  ...
  fetch('http://localhost:3000/token', {
    ...
    .then(function (data) {
      ...
      //   // }
      // }
      allBalls = data.state.allBalls
      playerNumber = data.state.playerNumber
