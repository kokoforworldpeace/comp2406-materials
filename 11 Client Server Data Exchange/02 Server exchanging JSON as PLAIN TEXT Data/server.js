...
JSON object, and acknowledge that, but still no attempt to
extract the data or parse it
...
http.createServer(function(request, response) {
  ...
  //POST message data can arrive in different chunks (messages)
  ...
  request.on('end', function() {
    ...
    if (request.method == "POST") {
      let dataObj = JSON.parse(receivedData) //PARSE DATA AS A JSON STRING
      console.log('received data object: ', dataObj)
      console.log('type: ', typeof dataObj)
      //HERE WE CREATE A JSON STRING BUT SEND IT AS PLAIN TEXT
      response.writeHead(200, {
        'Content-Type': MIME_TYPES['txt']
      })
      response.end('Data Received: ' + JSON.stringify(dataObj))
    }
...
console.log('Server Running at PORTR 3000  CNTL-C to quit')
console.log('To Test')
