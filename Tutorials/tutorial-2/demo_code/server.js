...
let fileObj = JSON.parse(fs.readFileSync('./html/Sister Golden Hair export.txt'))
for (var i = 0; i < fileObj.length; i++) {
  if (fileObj[i].chord) {
    xArray.push(fileObj[i])
  }
}
for (var i = 0; i < xArray.length; i++) {
    console.log(xArray[i].text);
  }
// fs.readFile('./html/Sister Golden Hair export.txt',(err,data)=>{
//   if (err) {
//     console.log(err)
//     return
//   }
...
//   xArray = JSON.parse(JSON.stringify(data))
//   // for (var i = 0; i < xArray.length; i++) {
//   //   console.log(xArray[i].text);
//   // }
// })
  ...
  request.on("end", function() {
    ...
    if (request.method == "POST") {
      ...
      if (songs.hasOwnProperty(dataObj.text)) {
        ...
        fs.readFile(songs[dataObj.text], (err,data)=>{
           if (err) {
             console.log(err)
             return
           }
           let lines_array = data.toString().replace(',','').split('\n')
           // let words_array = data.toString().replace(',','').split(' ')
           let result = ''
           let space = ' '
           let array = []
      ...
      console.log(`RETURN OBJECT :` + JSON.stringify(returnObj.wordArray))
