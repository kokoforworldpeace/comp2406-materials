...
1.html include script 'socket.io/socket.io.js'
2.create socket object using socket.io.js
i.e. let socket = io('http://' + window.document.location.host)// url
//io connection always on with above host url
3.socket.on('dataObj name',callback(data){ get feedback, update client side data })
//receive JSON string data
4.socket.emit('dataObj name',JSON data string)
...
let socket = io('http://' + window.document.location.host)// url
...
function handleMouseUp(e) {
  ...
  socket.emit('lyricsLocation',JSON.stringify(words))
}
...
//every 100 update moving box information
...
socket.on('BlueBoxData',function (data) {
  console.log(`CLIENT BOX DATA : ${data}`);
  let receivedJSON = JSON.parse(data)
  movingBox.x = receivedJSON.x
  movingBox.y = receivedJSON.y

})

socket.on('lyricsLocation',function (data) {
  console.log(`CLIENT LYRICS DATA : ${data}`);
  let receivedJSON = JSON.parse(data)
  words = receivedJSON
})
